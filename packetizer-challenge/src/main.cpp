...
class Client {
 public:
  Client() {
    ...
    UnixNetworkDevice device(Traits<UnixNetworkDevice>::ClientPort);
    ...
    while (true) {
      ...
      int result = device.send(buffer, Traits<UnixNetworkDevice>::ServerPort);
...
class Server {
 public:
  Server() {
    ...
    UnixNetworkDevice device(Traits<UnixNetworkDevice>::ServerPort);
