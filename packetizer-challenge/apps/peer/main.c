...
{
    (void)ctx;
    /* Check if the payload looks like a file transfer prefix */
...
static void *net_thread(void *arg)
{
    ...
    while (app->running) {
        /* select() with a short timeout so we can call pkt_tick() regularly */
        fd_set fds;
        FD_ZERO(&fds);
...
static int send_file(app_t *app, const char *path)
{
    ...
    /* Wait until the packetizer is free */
